...
| Qwen3.8-Flash-Next | GDN, QSA, gated residuals, PLE n-gram embeddings, FP8 and NVFP4 experts, optional FP8 dense projections, graph decode, prefix cache and MTP | FP8 deployment templates use two or four nodes. Single-node NVFP4 serving maps the n-gram table from NVMe. Batched decode supports sixteen rows, including eight slots at MTP depth 1; fixed-depth batches select fitting graph families by active request count, with scalar fallback when no family fits. Opt-in prefill continuation gives decode a turn between chunks |
...
image-aware prefix identities, the three-axis mRoPE positions that
let a vision tower keep its spatial layout, additional vision backends, audio and video
